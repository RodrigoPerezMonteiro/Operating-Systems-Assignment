	...
	if (v == NULL) {
		...
		for (i=0; i<NUM_EL; i++) {
			...
			if (strncmp(v,value,KEY_SIZE)!=0) {
				printf("Error on key %s value should be %s and was returned %s\n",key,value,v);
				exit(1);
			}
		...
		if (dim!=NUM_EL) {
			printf("TEST FAILED - SHOULD RETURN %d ELEMS AND HAS RETURNED %d\n",NUM_EL,dim);
			exit(-1);
		}
		...
		for (i=NUM_EL-1; i>=NUM_EL/2; i--) {
			...
			if (strncmp(v,value,KEY_SIZE)!=0) {
				printf("Error when removing key %s value should be %s and was returned %s\n",key,value,v);
				exit(1);
			}
		...
		for (i=0; i<NUM_EL; i++) {
			...
			if (i>=NUM_EL/2 && v!=NULL) {
				printf("Error when gettin key %s value should be NULL and was returned %s\n",key,v);
				exit(1);
			}
		...
		printf("----------------- %d-5/7 ENDED CHECKING AFTER REMOVE -----------------\n",client_id);
			...
			for (i=0; i<NUM_EL; i++) {
				...
				if (i>=NUM_EL/2 && v!=NULL) {
					printf("Error when getting key %s value should be NULL and was returned %s\n",key,v);
					exit(1);
				}
